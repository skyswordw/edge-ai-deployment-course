...
下图来自 [Hugging Face Course documentation-images dataset](https://huggingface.co/datasets/huggingface-course/documentation-images)，许可为 Apache-2.0。它适合直接放在推理基础章节，帮助学生先看到“输入处理、模型、输出处理”不是同一件事。
...
本课程后续改写时要把这张 NLP pipeline 接到 Qwen 本地服务链路：`messages` 先经过 chat template 和 tokenizer，再进入 llama.cpp runtime，最后通过 CLI 文本、HTTP JSON 或流式响应返回。
...
这张图适合提醒学生：tokenization 不是“文本长度统计”，而是会产生 token、offset、special tokens 等部署相关信息。课程实验里要把它转成 prompt token 数、ctx-size 和日志字段。
...
MLPerf, ONNX Runtime, TFLite/LiteRT, Nsight、Hugging Face pipeline 图和 llama-bench 都在强调同一件事: 先定义场景和测量边界, 再谈性能数字。本章直接嵌入许可清楚的 Hugging Face 原图；其他资料不复制 benchmark 表或工具截图, 而是重画为课程自己的推理记录流程。
...
- **图解**：直接嵌入 Hugging Face Apache-2.0 pipeline 原图，再把通用推理系统图重画成端到端延迟拆分图，单独标出模型加载、prefill、decode 和服务开销。
