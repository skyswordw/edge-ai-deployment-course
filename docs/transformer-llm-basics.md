...
以下原图来自 [Hugging Face Course documentation-images dataset](https://huggingface.co/datasets/huggingface-course/documentation-images)，许可为 Apache-2.0。本章先保留原图，后续可按本课程主线改写成中文图解。
...
这张图适合放在 Tokenizer 小节，用来说明文本进入模型前会先切成 token。课程改写时要补上中文、英文、代码和型号名的 token 数差异。
...
这张图适合解释 subword/BPE 的直觉：未知词不是直接失败，而是被拆成更小的片段。部署实验里它对应 prompt token 数、prefill 成本和上下文预算。
...
这张图适合解释 Transformer block 的重复堆叠。课程改写时只保留 attention、MLP、层数和权重量化之间的部署关系。
...
这张图适合做 Transformer 家族总览。本课程后续只重点展开 decoder-only LLM，因为 Qwen、GGUF、llama.cpp 和本地生成实验都围绕这一类模型。
...
这张图适合解释 decoder-only LLM 的训练和生成直觉：模型根据前文预测下一个 token。课程改写时要连接到 prefill/decode、`-n` 生成长度和停止条件。
...
这张图适合说明“预训练模型”和“面向任务的部署模型”不是一回事。本课程只保留它和微调、量化、部署回归有关的部分。
...
这张图适合解释基础 Transformer 表征和任务 head 的关系。对本课程来说，它提醒学生：不同任务形态可能改变 head、输出约束和后处理，不能只看主干模型文件大小。

这些原图进入本章后，建议立刻转成下面的部署字段，避免只停留在模型结构讲解：

| 原图/资料 | 本章保留的概念 | 后续实验字段 |
| --- | --- | --- |
| word / BPE tokenization | 字符数不等于 token 数 | prompt tokens、ctx-size、长输入风险 |
| Transformer blocks | 层、attention、MLP 是主要计算对象 | 模型大小、GPU offload、量化对象 |
| causal modeling | 生成是逐 token 预测 | `-n`、stop 条件、decode tokens/s |
| pretraining | 基座能力和任务适配不同 | 是否微调、adapter 去留 |
| transformer and head | 输出约束和后处理会随任务变化 | 质量检查、JSON 格式、API 响应 |

## 公开资料怎么转成本章内容

Attention 论文、Hugging Face chat template/KV Cache 文档、Qwen llama.cpp 教程和 vLLM PagedAttention 论文都在解释 LLM 的不同层面。本章直接嵌入许可清楚的 Hugging Face 原图；其他资料先重画为本课程图表, 并收束成一个部署契约: 输入格式要一致, token 数要可记录, prefill/decode 要分开看, KV Cache 要单独估算, 日志要能回到最终报告。
...
- **图解**：直接嵌入 Hugging Face Apache-2.0 原图作为参考，再把部署主线重画为“文本输入到采样输出”的推理链路图，突出 chat template 和 KV Cache。
