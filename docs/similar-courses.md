...
下面这些原图来自已经进入本课程资料池的公开来源。它们先作为“相似课程可以吸收什么”的视觉索引，后续章节再分别改写成课程讲解、Mermaid 图或实验表。
...
| 原图来源 | 本页吸收什么 | 课程落点 |
| --- | --- | --- |
| Hugging Face LLM Course | pipeline、tokenizer、模型和后处理的共同语言 | Part I、baseline、local API |
| DeepLearning.AI/vLLM | serving 课程结构、KV Cache、benchmark 和 metrics | Part V、profiling、报告 |
| Jetson AI Lab | 边缘设备形态和部署约束 | 环境矩阵、Jetson 实验 |
| ExecuTorch | 移动端/嵌入式 runtime 栈 | 移动端路线图 |
| Microsoft EdgeAI | local-first agent、工具和端云协同 | VLM/Agent 和系统复盘 |
...
外部资料进入本课程时，先过这张表。它把“可以参考的配图、知识点、实验细节”转成课程自己的图、表和任务。许可明确的原图可以直接贴入并标注来源；正文不整段照搬。
...
| --- | --- | --- | --- |
| 原始素材池 | Hugging Face documentation-images、Microsoft EdgeAI、vLLM 课程截图链接 | 许可明确的原图直接贴；许可不清的先保留原图链接并重画 | [可吸收原始资料暂存](/docs/raw-reference-intake) |
...
- **图解**：本章哪些原图可以直接贴，哪些图示思路要重画成课程图、表或流程。
...
| [OpenVINO NNCF](https://docs.openvino.ai/2025/openvino-workflow/model-optimization-guide/quantizing-models-post-training.html) | 官方教程 | PTQ、NNCF、Intel/OpenVINO 部署路径 | 用于多 runtime 对比 |
...
- 明确开放许可的原图可以直接嵌入并标注来源；许可不清的图只放链接或重画。
