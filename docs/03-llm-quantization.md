...
下面两张图来自 vLLM 官方博客中的 DeepLearning.AI/vLLM 课程截图。本章只借用它们的结构提示：KV Cache 要和权重分开看，量化方案要和服务化指标放在同一条链路里。
...
| 原图重点 | 本章吸收什么 | 本课程改写方向 |
| --- | --- | --- |
| KV Cache 是独立资源项 | 权重量化后，长上下文和多会话仍可能吃满内存 | `ctx-size`、prompt tokens、generated tokens、RAM/VRAM 必须进表 |
| 量化方案有多条路线 | GPTQ/AWQ/SmoothQuant/GGUF 不是同一个文件生态 | 把算法、格式、runtime 和设备支持分开比较 |
| 服务化课程把压缩和 benchmark 连起来 | 量化结论要进入 API 和报告，不停在离线文件大小 | Q8/Q5/Q4 -> llama-bench -> local API -> 风险登记 |
...
- **知识点**：从 Qwen、llama.cpp、KV cache 文档、DeepLearning.AI/vLLM serving 课程、Microsoft BitNet 和 GPTQ/AWQ/SmoothQuant/LLM.int8 中提取 GGUF、weight-only、outlier、imatrix、KV Cache、极低比特和 compress-serve-benchmark 边界。
- **图解**：远程贴入 vLLM/DeepLearning.AI KV Cache 和量化截图作为原图参考，再重画成 LLM 量化路径图、内存拆分图和质量风险表。
